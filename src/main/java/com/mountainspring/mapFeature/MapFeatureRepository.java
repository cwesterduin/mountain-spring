...
public interface MapFeatureRepository extends JpaRepository<MapFeature, UUID> {
    ...
    @Query(value = "SELECT m.id AS id, m.name AS name, m.pronunciation AS pronunciation, m.translation AS translation, m.type as type, m.coordinate AS coordinate, IF(m.primary_image_id IS NULL, 'false', 'true') AS primaryImage from map_feature as m", nativeQuery = true)
